...
using Logistics.Shared.Observability;
...
namespace Logistics.Modules.Capacity.Application
{
    /// <summary>
    /// Public API for the Capacity Module
    /// Implements capacity management: holds, confirmation, expiry
    /// </summary>
    public class CapacityApplicationService : ICapacityModule
    {
        ...
        private readonly ILogger<CapacityApplicationService> _logger;

        public CapacityApplicationService(DbContext db, VoyageCapacityRepository voyageRepo, IClock clock, ILogger<CapacityApplicationService> logger)
        {
        ...
        public async Task<CreateHoldResult> CreateHoldAsync(Guid bookingId, Guid voyageId, int units, TimeSpan ttl, string? idempotencyKey = null)
        {
            ...
            try
            {
                ...
                {
                    var existing = await _db.Set<IdempotencyEntryEntity>().FindAsync(idempotencyKey);
                    if (existing != null)
                    {
                        ...
                        // existing pending - treat as duplicate
                        await tx.CommitAsync();
                    ...
                    _db.Set<IdempotencyEntryEntity>().Add(idempotency);
                    await _db.SaveChangesAsync();
                }

                var booking = await _db.Set<BookingEntity>().FindAsync(bookingId);
                if (booking == null)
                {
                    if (!string.IsNullOrEmpty(idempotencyKey))
                    {
                ...
                {
                    var activeHold = await _db.Set<CapacityHoldEntity>().FindAsync(booking.ActiveHoldId.Value);
                ...
                var voyage = await _db.Set<VoyageCapacityEntity>().FindAsync(voyageId);
                if (voyage == null)
                {
                    if (!string.IsNullOrEmpty(idempotencyKey))
                    {
                ...
                // Try reserve atomically using repository
                ...
                _db.Set<CapacityHoldEntity>().Add(holdEntity);

                // Attach hold to booking
                booking.ActiveHoldId = holdId;
                booking.Version++;
                _db.Set<BookingEntity>().Update(booking);
                ...
                _db.Set<OutboxMessageEntity>().Add(outbox);
            ...
            catch (Exception ex)
            {
                ...
                if (!string.IsNullOrEmpty(idempotencyKey))
                {
                    ...
                    var existing = await _db.Set<IdempotencyEntryEntity>().AsNoTracking().SingleOrDefaultAsync(x => x.IdempotencyKey == idempotencyKey);
                    if (existing != null)
                    {
                        if (!string.Equals(existing.RequestHash, requestFingerprint, StringComparison.OrdinalIgnoreCase))
                            return new CreateHoldResult(false, null, "Idempotency key reused with different payload");
        ...
        public async Task<(bool Success, string? Reason)> ConfirmBookingAsync(Guid bookingId, Guid holdId, string? idempotencyKey = null)
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(idempotencyKey))
                {
                    ...
                    _db.Set<IdempotencyEntryEntity>().Add(new IdempotencyEntryEntity { IdempotencyKey = idempotencyKey, CreatedAt = dbNow, RequestHash = requestFingerprint, Status = "Pending" });
                    await _db.SaveChangesAsync();
                }
                ...
                var hold = await _db.Set<CapacityHoldEntity>().FindAsync(holdId);
                if (hold == null)
                {
                    if (!string.IsNullOrEmpty(idempotencyKey))
                    {
                ...
                _db.Set<CapacityHoldEntity>().Update(hold);
        ...
        {
            var hold = await _db.Set<CapacityHoldEntity>()
                .AsNoTracking()
        ...
        {
            var voyage = await _db.Set<VoyageCapacityEntity>()
                .AsNoTracking()
